...
const getPlaceByUserId = async (req, res, next) => {
  ...
  try {
    ...
    userWithPlace = await UserModel.findById(userId).populate('places');    
  } catch (error) {
    const errorObject = new HttpError(
  ...
  res
    .status(200)
    .json({ places: userWithPlace.places.map((place) => place.toObject({ getters: true })) });
};
...
const deletePlaceById = async (req, res, next) => {
  ...
  try {
    ...
    console.log(error);
    const errorObject = new HttpError(
      "Cannot delete the record, something went wrong",
      500
    );
    return next(errorObject);
  }
